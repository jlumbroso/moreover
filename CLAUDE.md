...
- **Project**: `moreover` — a non-interactive, cursor-based pager for readers who can't press space (LLMs, agents, scripts). **PUBLIC FROM DAY ONE** — Jérémie's first build-in-public: every commit is a publication, so publication gates apply to every byte (no estate-internal material beyond what is deliberately published here; no secrets ever; no transcripts or private working documents). Local session UUIDs in `docs/inbox/agent-sessions.json` are fine to publish — they are inert off-machine (filenames granting nothing) and the crew machinery requires them; commit-attribution `Claude-Session:` links are standard (auth-gated, provenance-aligned with building in public). Implementation: **Rust** (his ruling, "for io reasons" — ADR-0002). Estate coordination lives in the private commons HQ, never here.
